...
**Ограничение по версии:** миграции односторонние, поэтому тег обязан быть не
ниже той версии, что писала базу. Не выводите эту версию из mtime файла и дат
релизов на Docker Hub: в compose стоял `latest`, но образ скачали один раз
примерно за 24 месяца до последней записи и больше не обновляли — работавший
бинарник отставал от тега на два года. Реальная схема стала видна только при
первом старте: 0.63.2 применил 18 миграций начиная с `20241020003138`, то есть
база была старше октября 2024 (~0.53.x). Сейчас закреплено `0.63.2`.
...
Navidrome опознаёт медиа по пути внутри `/music`, поэтому класть в `Library/`
именно *содержимое* `ARTISTS/` (а не сам каталог `ARTISTS/`) — значит сохранить
пути, которые старая база уже знает. `Scanner.PurgeMissing` по умолчанию
`never`: то, что ещё не скопировано, попадает в *Missing Files*, а не удаляется,
и более поздняя копия всё ещё прицепится.

Первый запуск, 2026-09-13: 284 файла / 1.4 ГБ в 10 папках исполнителей,
сканирование 6.6 с, `tracksMissing=0`, подхвачен один плейлист `.m3u` (две его
записи с битой кодировкой пути не разрешились). `firstTime=false` — засеянные
учётные записи доехали, поэтому показывается вход, а не мастер первичной
настройки.
